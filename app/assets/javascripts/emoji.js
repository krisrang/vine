(function() {
  ...
  Object.keys(translations).forEach(function (code) {
    ...
    Vine.Dialect.inlineReplace(code, function (code) {
  ...
  if (Vine && Vine.EditorView) {
    Vine.EditorView.on("initWmdEditor", function(event){
     ...
     "<ul>" +
        "{{#each options}}" +
            "<li>" +
              "<a href='#'>" +
              "<img src='/images/emoji/{{this}}.png' class='emoji'> " +
              "{{this}}</a>" +
            "</li>" +
        "{{/each}}" +
     "</ul>" +
  ...
  Vine.Markdown.whiteListClass("emoji");
}).call(this);
