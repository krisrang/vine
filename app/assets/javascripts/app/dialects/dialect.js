/**
  ...
  Vine uses the Markdown.js as its main parser. `Vine.Dialect` is the framework
...
var parser = window.BetterMarkdown,
    ...
    dialect = MD.dialects.Vine = DialectHelpers.subclassDialect( MD.dialects.Gruber ),
    initialized = false,
    emitters = [];
...
function initializeDialects() {
  Vine.Dialect.trigger('register', {dialect: dialect, MD: MD});
  ...
  for (var j=1; j<node.length; j++) {
    ...
    if (typeof textContent === "string") {
      ...
      if (dialect.options.sanitize && !skipSanitize[textContent]) {
        textContent = Vine.Markdown.sanitize(textContent);
      }
...
function parseTree(tree, path, insideCounts) {
  ...
  if (tree instanceof Array) {
    ...
    Vine.Dialect.trigger('parseNode', event);
...
Vine.Dialect = {
  ...
  **/
  cook: function(text, opts) {
    ...
    var tree = parser.toHTMLTree(text, 'Vine'),
  ...
  /**
    ...
    ```javascript
      Vine.Dialect.inlineReplace(':)', function (text) {
  ...
  **/
  ...
  /**
    ...
    ```javascript
      ...
      Vine.Dialect.postProcessText(function (text) {
  ...
  **/
  ...
  /**
    ...
    ```javascript
      Vine.Dialect.postProcessTag('code', function (contents) {
...
RSVP.EventTarget.mixin(Vine.Dialect);
