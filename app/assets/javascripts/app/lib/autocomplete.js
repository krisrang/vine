...
$.fn.autocomplete = function(options) {
  ...
  var completeTerm = function(term) {
    if (term) {
      if (isInput) {
        ...
        me.val(text);
        Vine.Utilities.setCaretPosition(me[0], completeStart + 1 + term.length);
      }
    }
    closeAutocomplete();
  };
  ...
  $(this).keypress(function(e) {
    ...
    if (e.which === options.key.charCodeAt(0)) {
      var caretPosition = Vine.Utilities.caretPosition(me[0]);
  ...
  return $(this).keydown(function(e) {
    ...
    if ((completeStart === null) && e.which === 8 && options.key) {
      c = Vine.Utilities.caretPosition(me[0]);
      next = me[0].value[c];
    ...
    if (completeStart !== null) {
      caretPosition = Vine.Utilities.caretPosition(me[0]);
