...
module Oneboxer
  class AmazonOnebox < HandlebarsOnebox
    ...
    matcher /^https?:\/\/(?:www\.)?amazon.(com|ca|co.uk|de)\/.*$/
    favicon 'amazon.png'
    ...
    def translate_url
      ...
      return @url if @url =~ /https?:\/\/www\.amazon\.(com|ca|co.uk|de)\/gp\/aw\/d\//

      tld = URI.parse(@url).host.sub("www.", "").split(".")[1..-1].join(".")
      m = @url.match(/(?:d|g)p\/(?:product\/)?(?<id>[^\/]+)(?:\/|$)/mi)
      return "http://www.amazon.#{tld}/gp/aw/d/" + URI::encode(m[:id]) if m.present?
      @url
    end
