module CurrentUser
  def self.has_auth_cookie?(env)
    Vine.current_user_provider.new(env).has_auth_cookie?
  end

  def self.lookup_from_env(env)
    Vine.current_user_provider.new(env).current_user
  end

  # can be used to pretend current user does no exist, for CSRF attacks
  def clear_current_user
    @current_user_provider = Vine.current_user_provider.new({})
  end
  ...
  def current_user_provider
    @current_user_provider ||= Vine.current_user_provider.new(request.env)
  end
end